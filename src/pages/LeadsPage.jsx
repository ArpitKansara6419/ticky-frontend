...
function LeadsPage() {
  ...
  const calculateTicketTotal = (data) => {
    ...
    try {
      ...
      if ((startTime.includes('T') || startTime.includes(' ')) && !startTime.includes('Z') && !startTime.includes('+')) {
        const parts = startTime.split(/[T ]/);
        if (parts[1]) startHr = parseInt(parts[1].split(':')[0], 10);
      }
      if ((endTime.includes('T') || endTime.includes(' ')) && !endTime.includes('Z') && !endTime.includes('+')) {
